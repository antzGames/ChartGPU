      ...
      if (s.type === "pie") {
        for (let sliceIndex = 0; sliceIndex < s.data.length; sliceIndex++) {
          ...
          if (onSeriesToggle) {
            ...
            item.setAttribute(
              "aria-label",
              `Toggle ${getPieSliceLabel(slice?.name, sliceIndex)} visibility`,
            );
            item.tabIndex = 0;
          ...
          swatch.style.background = getPieSliceColor(
            slice?.color,
            seriesIndex,
            sliceIndex,
            theme,
        ...
        if (onSeriesToggle) {
          ...
          item.setAttribute(
            "aria-label",
            `Toggle ${getSeriesName(s, seriesIndex)} visibility`,
          );
          item.tabIndex = 0;
          item.dataset.seriesIndex = String(seriesIndex);
        }
